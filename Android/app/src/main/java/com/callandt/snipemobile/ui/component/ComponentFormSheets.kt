...
fun AddComponentSheet(
    ...
    ComponentFormSheet(
        viewModel = viewModel,
        title = L10n.string("new_component"),
...
fun EditComponentSheet(
    ...
    ComponentFormSheet(
        viewModel = viewModel,
        title = L10n.string("mgmt_edit_title", L10n.string("category_type_component")),
...
@Composable
private fun ComponentFormSheet(
    viewModel: AppViewModel,
    title: String,
    saveLabel: String,
    existing: Component?,
    ...
    var serial by remember(existing?.id) { mutableStateOf(existing?.decodedSerial.orEmpty()) }
    var modelNumber by remember(existing?.id) { mutableStateOf(existing?.decodedModelNumber.orEmpty()) }
    ...
    val componentCategories = remember(categories) { viewModel.apiClient.categoriesFor("component") }
    val categoryItems = remember(componentCategories) {
        componentCategories.map { PickerItem(it.id, it.decodedName) }
    }
    val canSave = name.trim().isNotEmpty() && selectedCategoryId > 0

    LaunchedEffect(Unit) {
        if (categories.isEmpty() || componentCategories.isEmpty()) viewModel.apiClient.fetchCategories()
    ...
    AssetFullScreenSheet(onDismiss = { if (!isSaving) onDismiss() }) {
        AssetFormSheetScaffold(
            ...
            onSave = {
                ...
                scope.launch {
                    ...
                    val success = if (existing == null) {
                        val result = viewModel.apiClient.createComponent(body)
                        if (!result.success) {
                            errorMessage = result.message ?: lastApiMessage ?: L10n.string("create_failed")
                        } else {
                            createdId = result.id
                            createdId?.let { viewModel.apiClient.fetchComponentDetails(it) }
                        }
                        result.success
                    } else {
                        val ok = viewModel.apiClient.updateComponent(existing.id, body)
            ...
            CreatableSearchablePickerField(
                ...
                createDefaults = mapOf("category_type" to "component"),
                onSelected = { selectedCategoryId = it.id },
            )
            OutlinedTextField(
                value = serial,
                onValueChange = { serial = it },
                label = { Text(L10n.string("serial")) },
                modifier = Modifier.fillMaxWidth(),
            )
            OutlinedTextField(
                value = modelNumber,
            ...
            FormSectionTitle(L10n.string("stock_usage"))
            OutlinedTextField(
                value = qtyText,
            ...
            if (hasPurchaseDate) {
                OutlinedTextField(
                    ...
                    label = { Text(L10n.string("purchase_date")) },
                    modifier = Modifier.fillMaxWidth(),
                )
            }

            FormSectionTitle(L10n.string("widget_other"))
            OutlinedTextField(
                value = notes,
