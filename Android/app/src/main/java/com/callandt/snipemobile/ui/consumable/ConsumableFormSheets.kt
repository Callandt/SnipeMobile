...
fun AddConsumableSheet(
    ...
    ConsumableFormSheet(
        viewModel = viewModel,
        title = L10n.string("new_consumable"),
...
fun EditConsumableSheet(
    ...
    ConsumableFormSheet(
        viewModel = viewModel,
        title = L10n.string("mgmt_edit_title", L10n.string("category_type_consumable")),
...
@Composable
private fun ConsumableFormSheet(
    viewModel: AppViewModel,
    title: String,
    saveLabel: String,
    existing: Consumable?,
    ...
    var itemNo by remember(existing?.id) { mutableStateOf(existing?.decodedItemNo.orEmpty()) }
    ...
    val consumableCategories = remember(categories) { viewModel.apiClient.categoriesFor("consumable") }
    val categoryItems = remember(consumableCategories) {
        consumableCategories.map { PickerItem(it.id, it.decodedName) }
    }
    val canSave = name.trim().isNotEmpty() && selectedCategoryId > 0

    LaunchedEffect(Unit) {
        if (categories.isEmpty() || consumableCategories.isEmpty()) viewModel.apiClient.fetchCategories()
    ...
    fun buildBody(): Map<String, Any?> {
        ...
        itemNo.trim().takeIf { it.isNotEmpty() }?.let { body["item_no"] = it }
    ...
    AssetFullScreenSheet(onDismiss = { if (!isSaving) onDismiss() }) {
        AssetFormSheetScaffold(
            ...
            onSave = {
                ...
                scope.launch {
                    ...
                    val success = if (existing == null) {
                        ...
                        val ok = viewModel.apiClient.updateConsumable(existing.id, body)
            ...
            CreatableSearchablePickerField(
                ...
                createDefaults = mapOf("category_type" to "consumable"),
                onSelected = { selectedCategoryId = it.id },
            )
            OutlinedTextField(
                value = itemNo,
                onValueChange = { itemNo = it },
                label = { Text(L10n.string("item_no")) },
                modifier = Modifier.fillMaxWidth(),
            )
            OutlinedTextField(
                value = modelNumber,
